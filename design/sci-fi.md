...
Arsitektur Sci-FI (Sci-FI) didirikan di atas pemisahan tugas kaku antara dua jenis mesin kecerdasan yang memiliki sifat komplementer:
...
### 1a. Taksonomi Sumbu Laten

Seluruh dimensi di runtime dipetakan ke dalam satu format representasi laten menggunakan sistem **skala unit** tanpa *hard clamp* (pemotongan batas ekstrem). 

Sumbu di runtime VM hanya dibedakan menjadi 2 jenis berdasarkan relasi grafis di Membrane:

#### 1. Linear (Default)
Dimensi linear di mana nilai laten ditransmisikan secara langsung dengan pembagian `unit_scale` sederhana:
$$\text{Nilai Laten} = \frac{\text{Nilai Sensor}}{\text{unit\_scale}}$$

* **Tanpa Clamping**: Nilai dibiarkan mengalir bebas. Boleh negatif (suhu di bawah nol, kecepatan mundur) dan boleh melebihi `1.0` (suhu ekstrem, beban berlebih). Target kisaran `[0.0, 1.0]` (atau `[-1.0, 1.0]`) diusahakan HLM semata-mata demi stabilitas numerik matriks di VM, bukan sebagai batas mati.
* **Penyederhanaan**: Kode VM tidak lagi membutuhkan fungsi normalize/percabangan pergeseran signed/unsigned.

| Dimensi | `unit_scale` | Contoh Output Runtime (Nilai Sensor / unit_scale) |
| --- | --- | --- |
| `0xMASS` | `10` | Sensor `5kg` → `0.5` |
| `0xTEMPERATURE` | `100` | Sensor `150°C` → `1.5` (Boleh > 1.0) |
| `0xWIND_SPEED` | `50` | Sensor `-10 m/s` (mundur) → `-0.2` (Boleh < 0.0) |

#### 2. Circular (Relasi ke `0xCIRCULAR`)
Dimensi siklik — diidentifikasi jika memiliki relasi grafis `derives_from: ["0xCIRCULAR"]` di Membrane.
* **Dual-Channel [sin, cos]**: Runtime Energen secara otomatis mengekspansi input sudut fisis 1-channel (radian) menjadi 2-channel laten $[0, 1]$ secara mulus:
  $$\text{channel}_{\sin} = \frac{\sin(\theta) + 1}{2}$$
  $$\text{channel}_{\cos} = \frac{\cos(\theta) + 1}{2}$$
* Ini menjaga agar kontinuitas $359^\circ$ dan $1^\circ$ tetap dekat secara matematis.

| Dimensi | Dimensi Sumber | Contoh Use Case |
| --- | --- | --- |
| `0xANGLE` | `0xCIRCULAR` | Orientasi sendi robot, arah angin |
| `0xPHASE` | `0xCIRCULAR` | Phase gelombang, siklus kimia |

> **Catatan:** Dimensi circular menghasilkan **2 nilai float** (sin + cos), bukan 1. Sasis membelah representasi ini secara otomatis.

---

### 1b. Spesifikasi Dimensi (Representasi DSL & JSON HLM)

Format spesifikasi dimensi yang dihasilkan oleh ekstraksi HLM:
...
```json
{
  "dimensions": [
    ...
    },
    {
      ...
      "proposed_id": "0x0011",
      "op": "special",
      "derives_from": ["0xCONSENSUS"]
    }
  ]
}
```

* **`0xCONSENSUS`**: Dimensi sistem global untuk pengawasan batas keketatan aman. Batas ini dihitung branchless via ReLU di ujung sirkuit: $\max(0.0, \text{actual} - \text{limit})$.

---

### 2. Arsitektur Aliran Injeksi Mesin (Zero-Hardcode Pipeline)

#### Langkah 1: Phase Ingesti & Ekstraksi (HLM & Ovipar)

* **Input:** Lembaran dokumen sains mentah (`docs` murni dari teks markdown, PDF, waveform).
* **Proses:** 
  1. **HLM (`mt-infer`)**: Membaca dokumen, menentukan dimensi, silsilah jangkar (`0xCIRCULAR` / `0xCONSENSUS`), dan mengusulkan `unit_scale` yang stabil.
  2. **Ovipar (Compiler)**: Menerima output JSON HLM, memvalidasi tipe/registry, menyolder matriks Yolk (*Yolk wiring*), dan melahirkan berkas biner `.egg`.
* **Output:** Berkas biner `.egg` yang berisi layout *embedding* (Albumin), causal mask (Membrane), dan bobot causal attention (Yolk) tanpa ada percabangan logika fisis di level compiler.
...
$$\text{Sci\_Attention}(Q,K,V) = \text{softmax}\left(\frac{Q \cdot K^T}{\sqrt{d_k}}\right)V$$

---

### 3. Matriks Simulasi Tabrakan Lintas Fenomena (Sci-FI)
...
#### Use Case N: Verifikasi Keaslian Logam Mulia & Deteksi Pemalsuan Material (Emas Verifier)

* **Query ($Q_{\text{0xMASS, 0xLENGTH, 0xCURRENT}}$):** Massa logam mulia (massa), volume geometri 3D benda ($L^3$, volume), dan kemampuan penghantar arus listrik (konduktivitas).
* **Key ($K_{\text{0xTIME, 0xTEMPERATURE, 0xAMPLITUDE}}$):** Kecepatan rambat frekuensi denting ketukan akustik ($1/T$, Hz), fluks perambatan panas (kapasitas kalor spesifik), dan batas nilai toleransi keaslian (kebijakan standar emas).
* **Mekanisme Otomatis:** Melalui *Self-Attention Loop*, model memproyeksikan korelasi multi-dimensi fisis antara massa jenis ($\rho = M/V$), konduktivitas listrik ($\sigma$), dan modulus elastisitas kecepatan suara ($v = \sqrt{E/\rho}$). Jika pemalsu menyisipkan logam dengan kerapatan mirip emas tapi konduktivitas rendah (seperti Tungsten), deviasi gabungan ini langsung terdeteksi di Yolk dan memicu status `INVALID` di output head *Membrane* tanpa memakan waktu pemrosesan laboratorium kimia yang lambat.
...
Induk dari AFB dan ADN. → [sci-fi-ads.md](sci-fi-ads.md)
...
Varian data stream untuk Sci-LM. → [sci-fi-afb.md](sci-fi-afb.md)

---

## Executable Graph Geometry (EGG)

Trained model. → [sci-fi-egg.md](sci-fi-egg.md)

---

## Ovipar

Sub-sistem/engine pelatihan Sci-LM. → [sci-fi-ovipar.md](sci-fi-ovipar.md)

---

## Energen

EGG runtime. → [sci-fi-energen.md](sci-fi-energen.md)

---

## Technology Stacks

Spesifikasi integrasi Bun (TypeScript) + Zig. → [sci-fi-stacks.md](sci-fi-stacks.md)

---

## Coder Validation

Dimensi dan mekanisme validasi kaku non-linter. → [sci-fi-coder.md](sci-fi-coder.md)
